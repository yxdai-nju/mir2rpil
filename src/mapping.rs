...
impl LowRpilMapping {
    ...
    pub fn insert_mappings_for_closure_call<'tcx>(
        ...
        let mapped_closure = LowRpilOp::Var {
            depth: depth + 1,
            index: 1,
        };
            ...
            mir::Operand::Move(closure_args_place) => {
                ...
                for (key, val) in self.mapping.iter() {
                    let place_index = match key {
                        ...
                        } if **base == args_place_op => place_index,
                        _ => continue,
                    };
                    let mapped_arg = LowRpilOp::Var {
                        depth: depth + 1,
                        index: place_index + 2,
                    };
    ...
    fn reduced_rpil_op(&self, op: &LowRpilOp) -> LowRpilOp {
        let structurally_reduced_op = match op {
            ...
            LowRpilOp::Ref(refed_op) | LowRpilOp::MutRef(refed_op) => match *refed_op.clone() {
